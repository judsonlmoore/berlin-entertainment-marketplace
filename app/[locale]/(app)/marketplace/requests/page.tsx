...
export default async function LeadsInboxPage({ params, searchParams }: Props) {
    ...
    <section className="grid gap-8">
        ...
        {STATUS_FILTERS.map((status) => {
          const href =
            status === "all"
              ...
              : `/marketplace/requests?status=${status}`;
      ...
      {pendingVenueEnquiries.length > 0 ? (
        <div className="panel grid gap-3 p-6">
          <h2 className="page-title text-xl">{t("pendingEnquiriesTitle")}</h2>
      ...
      <div className="panel grid gap-3 p-6">
        ...
        {leads.length === 0 ? (
          <p className="text-sm text-[var(--text-muted)]">{t("emptyLeads")}</p>
        ) : (
          <ul className="grid gap-3">
            {leads.map((lead) => (
              <li
              ...
              >
                ...
                <div className="mt-3">
                  <Link
                    href={`/marketplace/leads/${lead.bookingId}`}
      ...
      {can(access.actor, "direct_request.respond") ? (
        <div className="panel grid gap-3 p-6">
          <h2 className="page-title text-xl">{t("incomingRequestsTitle")}</h2>
          {incomingDr.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)]">
              {t("emptyIncomingRequests")}
            </p>
          ) : null}
          <ul className="grid gap-3">
      ...
      {operableVenueIds.length > 0 ? (
        <div className="panel grid gap-3 p-6">
          <h2 className="page-title text-xl">{t("outgoingRequestsTitle")}</h2>
          {outgoingDr.length === 0 && outgoingEnquiries.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)]">
              {t("emptyOutgoingRequests")}
            </p>
          ) : null}
          <ul className="grid gap-3">
