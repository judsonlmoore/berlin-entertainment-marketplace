...
The product succeeds when approved venues can find and confirm suitable acts, approved entertainers can find and win relevant opportunities, and platform staff can maintain marketplace trust through manual approval. Salon is not a consumer event platform, social network, payment custodian, or law firm.
...
- **Dual-role member:** one person who operates a venue and performs; one account holds both roles without duplicate identities. Dual-role users may use both discovery modes.
...
| `rejected` | Staff declined the application (optional retained denial) | No private marketplace access; history retained |

Only platform staff change approval state. Re-approval rules for material profile edits are an operational policy, not automatic identity verification. Approval of one marketplace role capability does not automatically approve the other role’s profile publication.
...
- Members may manage venue workflows granted by the permission set, but cannot remove the last owner or change platform approval.
...
Optional: website/social links, house rules, venue images, multiple spaces with distinct capacity/resources. A venue profile is discoverable only to approved **entertainers** (and staff) after staff publication approval.
...
Optional: accessibility notes, languages, equipment supplied, website/social/YouTube links, portfolio media, technical rider uploads. Prices are indicative; agreed booking terms are authoritative. An entertainer profile is discoverable only to approved **venue operators** (and staff) after staff publication approval.
...
- Only approved accounts access private marketplace surfaces.
- **Role segregation (server-enforced):**
  - Entertainers may search approved venues, venue spaces, and open opportunities only. They must not browse, search, or open other entertainers’ profiles (except their own).
  - Venues may search approved entertainer profiles only. They must not browse, search, or open other venues’ private profiles, except where already a party to a shared booking involving that venue.
  - Dual-role members may use both discovery modes. Staff may access both for moderation.
